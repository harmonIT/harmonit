...
func dialTestConn(r string, w io.Writer) DialOption {
	return DialNetDial(func(network, addr string) (net.Conn, error) {
...
func dialTestConnTLS(r string, w io.Writer) DialOption {
	return DialNetDial(func(network, addr string) (net.Conn, error) {
...
func TestRead(t *testing.T) {
	for _, tt := range readTests {
		c, _ := Dial("", "", dialTestConn(tt.reply, nil))
...
func TestDialURL(t *testing.T) {
	for _, tt := range dialURLTests {
		...
		_, err := DialURL(tt.url, dialTestConn(tt.r, &buf), DialUseTLS(true))
		if err != nil {
...
func checkPingPong(t *testing.T, buf *bytes.Buffer, c Conn) {
	resp, err := c.Do("PING")
	if err != nil {
...
func TestDialURLTLS(t *testing.T) {
	var buf bytes.Buffer
	c, err := DialURL("rediss://example.com/",
		DialTLSConfig(&clientTLSConfig),
...
func TestDialUseTLS(t *testing.T) {
	var buf bytes.Buffer
	c, err := Dial("tcp", "example.com:6379",
		DialTLSConfig(&clientTLSConfig),
		dialTestConnTLS(pingResponse, &buf),
		DialUseTLS(true))
	if err != nil {
...
func TestWithTimeout(t *testing.T) {
	for _, recv := range []bool{true, false} {
		for _, defaultTimout := range []time.Duration{0, time.Minute} {
			...
			c, _ := Dial("", "", DialReadTimeout(defaultTimout), DialNetDial(func(network, addr string) (net.Conn, error) { return nc, nil }))
			for i := 0; i < 4; i++ {
				...
				if i%2 == 0 {
					...
					if recv {
						ReceiveWithTimeout(c, timeout)
					} else {
						DoWithTimeout(c, timeout, "PING")
					}
