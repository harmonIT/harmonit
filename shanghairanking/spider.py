...
resp.encoding='utf-8'#解压gzip压缩格式为utf-8
parse=json.loads(resp.text)
rankings=parse['data']['rankings']
grade=[i['grade'] for i in rankings]
ranking=[i['ranking'] for i in rankings]
univNameCn=[i['univNameCn'] for i in rankings]
univTags=[i['univTags'] for i in rankings]
city=[i['city'] for i in rankings]
score=[i['score'] for i in rankings]
pdData={
    '评级':grade[:50],
    '排名':ranking[:50],
    '学校名称':univNameCn[:50],
...
df.to_excel('中国大学专业网络与新媒体2024年学校排名.xlsx',index=False)
# print(grade[1])
